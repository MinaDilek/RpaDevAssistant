...
public sealed class UiPathConfigAnalysisService : IUiPathConfigAnalysisService
{
    ...
    {
        var environmentComparisons = BuildEnvironmentComparisons(entries);
        var safeEntries = entries.Select(SanitizeEnvironmentValues).ToArray();
        return new UiPathConfigAnalysisResult
        {
            ...
            Usages = usages,
            UnusedKeys = unused
                .Select(item => item with { Entry = SanitizeEnvironmentValues(item.Entry) })
                .ToArray(),
            MissingKeys = missing,
            ...
            Overview = new UiPathConfigAnalysisOverview
            {
                ...
                EnvironmentCount = entries
                    .SelectMany(entry => entry.EnvironmentValues.Keys)
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .Count(),
                EnvironmentIssueCount = environmentComparisons.Count(comparison => comparison.HasIssue)
            }
        };
    }

    private static UiPathConfigEntry SanitizeEnvironmentValues(UiPathConfigEntry entry)
    {
        return IsSensitiveConfigKey(entry.Key) && entry.EnvironmentValues.Count > 0
            ? entry with
            {
                EnvironmentValues = entry.EnvironmentValues.ToDictionary(
                    pair => pair.Key,
                    pair => string.IsNullOrWhiteSpace(pair.Value) ? null : "[REDACTED]",
                    StringComparer.OrdinalIgnoreCase)
            }
            : entry;
    }

    private static IReadOnlyList<UiPathConfigEnvironmentComparison> BuildEnvironmentComparisons(
        IReadOnlyList<UiPathConfigEntry> entries)
    {
        var environments = entries
            .SelectMany(entry => entry.EnvironmentValues.Keys)
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .OrderBy(EnvironmentSortOrder)
        ...
        return entries
            .Where(entry => entry.EnvironmentValues.Count > 0)
            .GroupBy(entry => entry.Key, KeyComparer)
            .OrderBy(group => group.Key, StringComparer.OrdinalIgnoreCase)
            .Select(group =>
            {
                var values = environments.ToDictionary(
                    environment => environment,
                    environment => group
                        .Select(entry => entry.EnvironmentValues.TryGetValue(environment, out var value) ? value : null)
                ...
                var distinctValues = values.Values
                    ...
                    .Count();
                var duplicateEnvironmentConflict = environments.Any(environment =>
                    group
                        .Select(entry => entry.EnvironmentValues.TryGetValue(environment, out var value) ? value : null)
                        .Where(value => !string.IsNullOrWhiteSpace(value))
                        .Distinct(StringComparer.Ordinal)
                        .Skip(1)
                        .Any());
                return new UiPathConfigEnvironmentComparison
                {
                    Key = group.Key,
                    Values = IsSensitiveConfigKey(group.Key)
                        ? values.ToDictionary(
                            pair => pair.Key,
                            pair => string.IsNullOrWhiteSpace(pair.Value) ? null : "[REDACTED]",
                            StringComparer.OrdinalIgnoreCase)
                    ...
                    HasDifferentValues = distinctValues > 1 || duplicateEnvironmentConflict
                };
            })
            .ToArray();
    }

    private static int EnvironmentSortOrder(string environment) => environment.ToUpperInvariant() switch
    {
        "DEV" => 0,
        "TEST" => 1,
        "UAT" => 2,
    ...
    {
        var compact = Regex.Replace(key, "[^A-Za-z0-9]", string.Empty);
        return new[] { "password", "pwd", "secret", "token", "apikey", "clientsecret", "credential" }
            .Any(term => compact.Contains(term, StringComparison.OrdinalIgnoreCase));
    }
        ...
        IReadOnlyList<string> sharedStrings)
    {
        ...
        var environmentColumns = headerCells
            .Where(cell => IsEnvironmentHeader(cell.Value))
            .ToDictionary(cell => NormalizeEnvironmentName(cell.Value), cell => cell.Column, StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var row in rows.Skip(1))
        {
            ...
            entries.Add(new UiPathConfigEntry
            {
                ...
                SheetName = sheetName,
                RowNumber = rowNumber,
                EnvironmentValues = environmentColumns.ToDictionary(
                    pair => pair.Key,
                    pair => cells.TryGetValue(pair.Value, out var environmentCell)
                        ? NullIfEmpty(ReadCellValue(environmentCell, sharedStrings))
                        : null,
                    StringComparer.OrdinalIgnoreCase)
            });
        }
    ...
    private static bool IsEnvironmentHeader(string value)
    {
        return NormalizeEnvironmentName(value) is "DEV" or "TEST" or "UAT" or "STAGING" or "PROD";
    }

    private static string NormalizeEnvironmentName(string value)
    {
        return value.Trim().Replace(" ", string.Empty, StringComparison.Ordinal).ToUpperInvariant() switch
        {
            "DEVELOPMENT" => "DEV",
            "QA" or "QUALITYASSURANCE" => "TEST",
            "STAGE" => "STAGING",
            "PRODUCTION" => "PROD",
