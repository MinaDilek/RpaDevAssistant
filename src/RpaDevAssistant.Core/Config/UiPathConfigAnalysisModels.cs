...
public sealed record UiPathConfigAnalysisResult
{
    ...
    public IReadOnlyList<UiPathConfigEnvironmentComparison> EnvironmentComparisons { get; init; } = [];
...
public sealed record UiPathConfigAnalysisOverview
{
    ...
    public int EnvironmentIssueCount { get; init; }
}
...
public sealed record UiPathConfigEnvironmentComparison
{
    public required string Key { get; init; }
    ...
    public IReadOnlyList<string> MissingEnvironments { get; init; } = [];

    public bool HasDifferentValues { get; init; }

    public bool HasIssue => MissingEnvironments.Count > 0 || HasDifferentValues;
}
