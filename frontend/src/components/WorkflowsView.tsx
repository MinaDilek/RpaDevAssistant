...
import { isTauriDesktop, openWorkflowInStudio } from '../services/projectFolderService';
import {
  Metric,
...
export function WorkflowsView({
  ...
  const [studioOpenError, setStudioOpenError] = React.useState<string | null>(null);
  const [studioOpening, setStudioOpening] = React.useState(false);
  ...
  async function handleOpenInStudio() {
    if (!selectedWorkflow || !projectPath || !isTauriDesktop()) return;
    setStudioOpening(true);
    setStudioOpenError(null);
    try {
      await openWorkflowInStudio(projectPath, selectedWorkflow.relativePath);
    } catch (error) {
      setStudioOpenError(error instanceof Error ? error.message : t('openInStudioFailed'));
    } finally {
    ...
    <div className={`workflows-layout ${selectedWorkflow ? 'with-drawer' : 'single-column'}`}>
      ...
      {selectedWorkflow && selectedDetail && (
        <>
          ...
          <div className="section-header">
            ...
            <div className="history-actions">
              <button
                ...
                disabled={!isTauriDesktop() || studioOpening}
                title={!isTauriDesktop() ? t('desktopOnlyAction') : undefined}
              >
                <ExternalLink size={16} />
                {studioOpening ? t('opening') : t('openInUiPathStudio')}
              </button>
          ...
          {studioOpenError && <p className="error-text">{studioOpenError}</p>}
          ...
          <details open>
            ...
            {selectedDetail.arguments.length === 0 ? (
              ...
              <ArgumentContract arguments={selectedDetail.arguments} t={t} />
            )}
          </details>
          <details open>
          ...
          <details open>
            ...
            {selectedDetail.activities.length === 0 ? (
              ...
              <ActivityTree activities={selectedDetail.activities} t={t} />
            )}
          </details>
          <details open>
...
export function buildInvocationGraph(
  workflows: ReturnType<typeof getWorkflowHealth>,
...
export function ProjectArchitecture({
  ...
  onSelectWorkflow: (workflow: ReturnType<typeof getWorkflowHealth>[number]) => void;
  t: (key: string, values?: Record<string, unknown>) => string;
}) {
  const [expanded, setExpanded] = React.useState(false);
  const summary = React.useMemo(() => {
    const allInvocations = Array.from(invocationGraph.values()).flat();
    return {
      staticEdges: allInvocations.filter((invoke) => !invoke.isDynamic && invoke.targetWorkflow).length,
      dynamicEdges: allInvocations.filter((invoke) => invoke.isDynamic).length,
      danglingEdges: allInvocations.filter((invoke) => !invoke.isDynamic && !invoke.targetWorkflow).length,
    };
  }, [invocationGraph]);
  ...
  return (
    ...
    >
      <summary>
        <span><GitBranch size={16} />{t('projectArchitecture')}</span>
        <small>{t('architectureSummary', { workflows: workflows.length, invokes: summary.staticEdges })}</small>
      </summary>
      {expanded && <><div className="architecture-metrics" aria-label={t('architectureMetrics')}>
        <span><strong>{workflows.length}</strong>{t('workflowNodes')}</span>
        <span><strong>{summary.staticEdges}</strong>{t('staticInvocations')}</span>
        <span><strong>{summary.dynamicEdges}</strong>{t('dynamicInvocations')}</span>
        <span className={summary.danglingEdges > 0 ? 'risk' : ''}>
          <strong>{summary.danglingEdges}</strong>{t('danglingInvocations')}
        </span>
      </div>
      <div className="architecture-map" role="list" aria-label={t('projectArchitecture')}>
        {workflows.map((workflow) => {
          const invokes = invocationGraph.get(normalizeWorkflowPath(workflow.relativePath)) ?? [];
          return (
            <article className="architecture-node" role="listitem" key={workflow.relativePath}>
              <button type="button" onClick={() => onSelectWorkflow(workflow)}>
                {workflow.relativePath}
              </button>
              {invokes.length === 0 ? (
                <small>{t('noOutgoingInvocations')}</small>
              ) : (
                <ul>
                  {invokes.map((invoke, index) => (
                    <li
                      className={invoke.isDynamic ? 'dynamic' : invoke.targetWorkflow ? 'resolved' : 'dangling'}
                      key={`${invoke.sourceActivityId ?? index}-${invoke.rawPath}`}
                    >
                      <span aria-hidden="true">→</span>
                      {invoke.targetWorkflow ? (
                        <button type="button" onClick={() => onSelectWorkflow(invoke.targetWorkflow!)}>
                          {invoke.targetWorkflow.relativePath}
                        </button>
                      ) : (
                        <span>
                          {invoke.isDynamic
                            ? t('dynamicWorkflowReference')
                            : invoke.normalizedPath || t('unknown')}
                        </span>
                      )}
                    </li>
                  ))}
...
function ArgumentContract({
  ...
  arguments: Array<{ name: string; direction?: string | null; type?: string | null }>;
  t: (key: string, values?: Record<string, unknown>) => string;
}) {
  const counts = workflowArguments.reduce(
    (summary, argument) => {
      summary[classifyArgumentDirection(argument.direction)] += 1;
      return summary;
    },
    { input: 0, output: 0, inOut: 0, unknown: 0 },
  );

  return (
    <div className="argument-contract">
      <div className="argument-contract-header">
        <div>
          <strong>{t('argumentContract')}</strong>
          <p>{t('argumentContractHelp')}</p>
        </div>
        <span className="argument-total">{t('argumentCountLabel', { count: workflowArguments.length })}</span>
      </div>
      <div className="argument-contract-summary" aria-label={t('argumentContractSummary')}>
        <span><strong>{counts.input}</strong>{t('inputArguments')}</span>
        <span><strong>{counts.output}</strong>{t('outputArguments')}</span>
        <span><strong>{counts.inOut}</strong>{t('inOutArguments')}</span>
        {counts.unknown > 0 && <span><strong>{counts.unknown}</strong>{t('unknownDirection')}</span>}
      </div>
      <div className="table-scroll">
        <table className="compact-table argument-contract-table">
          <thead>
            <tr>
              <th>{t('name')}</th>
              <th>{t('direction')}</th>
              <th>{t('contractRole')}</th>
              <th>{t('type')}</th>
            </tr>
          </thead>
          <tbody>
            {workflowArguments.map((argument) => {
              const role = classifyArgumentDirection(argument.direction);
              return (
                <tr key={`${argument.name}-${argument.direction ?? ''}`}>
                  <td><code>{argument.name}</code></td>
                  <td>{argument.direction ?? '-'}</td>
                  <td><span className={`argument-role ${role}`}>{t(`argumentRole_${role}`)}</span></td>
...
function classifyArgumentDirection(direction?: string | null): 'input' | 'output' | 'inOut' | 'unknown' {
  const normalized = (direction ?? '').replace(/[\s/_-]/g, '').toLowerCase();
  if (normalized === 'in' || normalized === 'input') return 'input';
  if (normalized === 'out' || normalized === 'output') return 'output';
  if (normalized === 'inout' || normalized === 'inputoutput') return 'inOut';
  return 'unknown';
}
...
export function ActivityTree({
  ...
  const visibleIds = React.useMemo(() => {
    ...
    for (const activity of activities) {
      const searchableValues = [
        ...
        ...Object.values(activity.properties ?? {}),
      ];
      if (!searchableValues.some((value) => String(value ?? '').toLocaleLowerCase().includes(normalizedQuery))) {
        continue;
      }
  ...
  const matchCount = React.useMemo(() => {
    if (!normalizedQuery) return activities.length;
    return activities.filter((activity) => activityMatchesQuery(activity, normalizedQuery)).length;
  }, [activities, normalizedQuery]);

  return (
    <div className="activity-tree-panel" role="region" aria-label={t('activityTree')}>
      <div className="activity-tree-search">
        <input
          type="search"
          aria-label={t('searchActivityTree')}
          placeholder={t('searchActivityTreePlaceholder')}
          value={query}
          onChange={(event) => setQuery(event.target.value)}
        />
        {query && (
          <button type="button" onClick={() => setQuery('')} aria-label={t('clearActivitySearch')}>
            {t('clear')}
          </button>
        )}
      </div>
      {normalizedQuery && (
        <p className="activity-search-result" aria-live="polite">
          {t('activitySearchResult', { count: matchCount })}
        </p>
      )}
      {visibleIds?.size === 0 ? (
        <p className="empty-state">{t('noMatchingActivities')}</p>
      ) : (
        <ul className="activity-tree">
          {roots.filter((activity) => !visibleIds || visibleIds.has(activity.activityId)).map((activity) => (
            <ActivityTreeNode
...
function activityMatchesQuery(activity: Activity, normalizedQuery: string): boolean {
  return [
    activity.name,
    activity.displayName,
    activity.typeName,
...
export function ActivityTreeNode({
  ...
  const children = (childrenByParent.get(activity.activityId) ?? []).filter(
    (child) => !visibleIds || visibleIds.has(child.activityId),
  );
  ...
  if (children.length === 0) {
    return (
      <li>
        <div>
          <span>{label}</span>
          <ActivityProperties activity={activity} t={t} />
        </div>
      </li>
    );
  }
  ...
  return (
    <li>
      <details open={forceExpanded || expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
...
export function ActivityProperties({
  activity,
  t = (key, values) => translate('en', key, values),
}: {
  activity: Activity;
