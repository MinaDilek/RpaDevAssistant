...
export function RulesView({
  ...
  const moduleInputRef = React.useRef<HTMLInputElement | null>(null);
  ...
  async function downloadModule() {
    try {
      const module = await exportRuleModule({ moduleId: 'local.rpadevassistant.rules', name: 'RPA Dev Assistant Rule Module', version: '1.0.0', publisher: 'Local' });
      downloadJson('rpa-rule-module.json', module);
      onStatus(t('ruleModuleExported'));
    } catch (error) {
      onStatus(error instanceof Error ? error.message : t('ruleModuleExportFailed'));
    }
  }

  async function importModuleFromFile(file: File) {
    try {
      await importRuleModule(JSON.parse(await file.text()), false);
      onStatus(t('ruleModuleImported'));
      await loadCustomRules();
      onRefresh();
    } catch (error) {
      onStatus(error instanceof Error ? error.message : t('ruleModuleImportFailed'));
    }
  }
    ...
    <section className="results-panel rules-panel" aria-label={t('rules')}>
      <div className="section-heading rules-page-heading">
        ...
        <div className="action-row compact-actions">
          ...
          <button type="button" className="icon-text-button" onClick={() => void downloadModule()}>
            <Download size={16} />
            {t('exportModule')}
          </button>
          <button type="button" className="icon-text-button" onClick={() => moduleInputRef.current?.click()}>
            <Upload size={16} />
            {t('importModule')}
          </button>
          <button
          ...
          <input
            ...
            onChange={(event) => {
              const file = event.target.files?.[0];
              event.target.value = '';
              if (file) void importModuleFromFile(file);
            }}
          />
        </div>
      </div>
