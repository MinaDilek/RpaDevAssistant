...
export function ConfigAnalysisView({
  ...
  const [section, setSection] = React.useState<
    'overview' | 'graph' | 'environments' | 'unused' | 'missing' | 'hardcoded' | 'preview'
  >('overview');
        ...
        <>
          <nav className="config-section-nav" aria-label={t('configAnalysis')}>
            ...
            <button
              ...
              aria-label={t('environmentConfig')}
            >
              <GitCompareArrows size={19} />
              <span>{t('environmentConfig')}</span>
              <strong>{result.overview.environmentIssueCount}</strong>
            </button>
            <button
              type="button"
          ...
          {section === 'environments' && (
            <ConfigEnvironmentTable items={result.environmentComparisons ?? []} t={t} />
          )}
...
export function ConfigEnvironmentTable({
  items,
  t,
}: {
  items: ConfigEnvironmentComparison[];
  t: (key: string, values?: Record<string, unknown>) => string;
}) {
  ...
  return (
    <section className="panel-card config-environment-panel">
      <h2>{t('environmentConfig')}</h2>
      <p className="hint">{t('environmentConfigHelp')}</p>
      {items.length === 0 ? (
        <p className="empty-state">{t('noEnvironmentConfig')}</p>
      ) : (
        <div className="table-scroll-region">
          <table>
            <thead>
              <tr>
                <th>{t('configKey')}</th>
                {environments.map((environment) => <th key={environment}>{environment}</th>)}
                <th>{t('status')}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr key={item.key}>
                  ...
                  {environments.map((environment) => (
                    <td key={`${item.key}-${environment}`}>
                      <code>{item.values[environment] ?? t('missing')}</code>
                    </td>
                  ))}
                  <td>
                    <span className={`environment-status ${item.hasIssue ? 'warning' : 'ok'}`}>
                      {item.missingEnvironments.length > 0
                        ? t('environmentValuesMissing', { environments: item.missingEnvironments.join(', ') })
                        : item.hasDifferentValues
                          ? t('environmentValuesDiffer')
                          : t('environmentValuesConsistent')}
                    </span>
                  </td>
                </tr>
              ))}
