...
{
    [Fact]
    public void Analyze_ComparesEnvironmentColumnsAndRedactsSensitiveValues()
    {
        using var project = ConfigProjectFixture.Create();
        project.WriteEnvironmentConfig(
            ("ApiUrl", "https://dev", "https://test", "https://prod"),
            ("RetryCount", "2", "", "4"),
            ("ClientSecret", "dev-secret", "test-secret", "prod-secret"));

        var result = CreateService().Analyze(project.RootPath);

        Assert.Equal(3, result.Overview.EnvironmentCount);
        Assert.Equal(3, result.Overview.EnvironmentIssueCount);
        var retry = Assert.Single(result.EnvironmentComparisons, item => item.Key == "RetryCount");
        Assert.Contains("TEST", retry.MissingEnvironments);
        Assert.True(retry.HasDifferentValues);
        var secret = Assert.Single(result.EnvironmentComparisons, item => item.Key == "ClientSecret");
        Assert.All(secret.Values.Values, value => Assert.Equal("[REDACTED]", value));
        Assert.DoesNotContain("dev-secret", System.Text.Json.JsonSerializer.Serialize(result));
    }

    [Fact]
    public void Analyze_DoesNotCreateEnvironmentComparisonsForStandardValueWorkbook()
    {
        using var project = ConfigProjectFixture.Create();
        project.WriteConfig(("ApiUrl", "https://example.test"));

        var result = CreateService().Analyze(project.RootPath);

        Assert.Empty(result.EnvironmentComparisons);
        Assert.Equal(0, result.Overview.EnvironmentCount);
        Assert.Equal(0, result.Overview.EnvironmentIssueCount);
    }
    ...
    private sealed class ConfigProjectFixture : IDisposable
    {
        ...
        public void WriteEnvironmentConfig(params (string Key, string Dev, string Test, string Prod)[] values)
        {
            WriteConfig(values.Select(value => (value.Key, value.Dev)).ToArray());
            var path = Path.Combine(RootPath, "Data", "Config.xlsx");
            using var archive = ZipFile.Open(path, ZipArchiveMode.Update);
            var rows = new List<string>
            {
                Row(1, ("A", "Name"), ("B", "DEV"), ("C", "TEST"), ("D", "PROD"), ("E", "Description"))
            };
            rows.AddRange(values.Select((value, index) => Row(
                index + 2,
                ("A", value.Key),
                ("B", value.Dev),
                ("C", value.Test),
                ("D", value.Prod),
                ("E", string.Empty))));
            ReplaceText(archive, "xl/worksheets/sheet1.xml", $$"""
